...
Tanmay Jain · CERA Lab · August 28, 2026 · Branch `feat/dynamic-flood-maps` (commit `91f44a7`)
...
5. **Not yet run:** ±3–6 h arrival perturbation; Sandy hindcast timeline check (ConEd 21:00, NYU Langone 19:30–23:00, tunnels, FCC).
...
**Dr. Miura**
1. *When should the telecom demand surge be anchored — storm peak (current, Sandy-consistent) or storm onset?* This one modeling choice decides whether the saturated-regime damage is +21–58 % and persistent, or mean-neutral with a fat bimodal tail. Either is a finding; the memo needs to know which one to lead with.
...
`analysis/static_vs_dynamic_v1/campaign_table_v1.{md,csv}` (23 rows), `fig_timing_effect_vs_level_v1.png`, `fig_prepeak_failures_v1.png`; three-storm N = 1,000 tables `arrival_vs_staticpeak_h360_n1000_v1.csv`, `arrival_clockonset_vs_staticpeak_h360_n1000_v1.csv`; `analysis/gnn_twin_v1/twin_heldout_v1.{md,csv}`, `fig_twin_cascade_pr_v1.png`, `fig_twin_val_loss_v1.png`; `analysis/TwinCampaign_20maps_resolved_v1.csv`; `analysis/gissr_delivered_maps_audit_v1.txt`; timing tables `data/flood/timing/node_timing_jesse22_v1{,_summary}.csv`. Dynamic campaign outputs: Torch `$SCRATCH/results/dynamic_v1/` and Mac `data/hpc_results_aug2026/dynamic_v1/` (46 files).

## 4.6 Arrival-error robustness (v2 addition)

Arrival-error robustness (±3/±6 h uniform shifts, N=100, 3 storms): no shift changes any crash decision — crash fractions are identical in every arm. End-state deltas are ≤2.2% (914_6) and ≤0.4% (605_5); the crash-edge storm 880_9 is asymmetrically sensitive: uniformly earlier arrival raises the end state by up to +13% (+315 nodes; pre-peak failures swing 370 → 988 across the +6 → −6 h span) while later arrival moves it ≤2%. The when/how-much conclusions are robust to arrival error except for an early-bias sensitivity confined to the tipping zone; the whether conclusion is untouched.
