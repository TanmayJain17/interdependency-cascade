...
def main():
    ...
    ap.add_argument("--depth-csv", default="data/flood/jesse22_node_depths_v1.csv",
                    help="fallback depth source (flood_<scenario>_depth_m column) when the campaign temp_nodes geojson is absent, e.g. the GeoClaw trio")
    a = ap.parse_args()
    ...
    if a.mode != "static":
        # never let a dynamic run silently degrade to static forcing (a scenario missing from the timing table)
        from dynamic_forcing import load_dynamic_forcing
        dyn = load_dynamic_forcing()
        if dyn is None or not dyn.has_scenario(a.scenario):
            sys.exit(f"mode={a.mode} requested but {a.scenario} has no timing table entry — refusing to fall back to static")
    ...
    if not ref_runs.exists():
        sys.exit(f"missing campaign file: {ref_runs}")
    ...
    if ref_nodes.exists():
        ...
        src_name = ref_nodes.name
    else:
        import pandas as pd
        dcsv = pd.read_csv(a.depth_csv)
        if col not in dcsv.columns:
            sys.exit(f"{ref_nodes} absent and {a.depth_csv} has no column {col}")
    ...
    print(f"nodes {len(nodes_gdf):,} from {msr.NODES_IN} | depths from {src_name}: wet {wet}, unmatched ids {missing}")
    if missing:
